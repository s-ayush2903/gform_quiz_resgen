...
pwd = os.getcwd()
fle = os.path.join(pwd, "uploads\\sample.csv")
master = os.path.join(pwd, "uploads\\master.csv")
...
def prepareQuizResult(rollNo, line=[], absent=False):
    ...
    #sheet.add_image(Image("instiLogo.jpeg"), "A1")
        ...
        for col in colls:
            ...
            if col == "B":
                if inr == 10:
                    ...
                    sheet[col + str(inr)] = sheet["B10"].value * sheet["B11"].value
    ...
    sheet["E12"] = (
        f"{cors * corPoints - (wrong * incorPoints)}/{(cors + left + wrong) * corPoints}"
        if not absent
        else "Absent"
    )
